...
if [ -f "$APP_ROOT/manage.py" ]; then
  ...
  python manage.py seed_trust_categories   || echo "seed_trust_categories failed (continuing)."
  python manage.py seed_operations_metrics || echo "seed_operations_metrics failed (continuing)."
